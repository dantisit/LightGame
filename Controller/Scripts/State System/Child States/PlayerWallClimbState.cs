...
public class PlayerWallClimbState : MainState, IMove1D
{
    ...
    {
        base.Enter();
        rigidbody.gravityScale = playerData.Walls.Physics2DGravityScale;
        localTime = 0f;
        curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallClimb.SpeedUpCurve, rigidbody.linearVelocity.x / playerData.Walls.WallClimb.MaxSpeed, 1, true);
    ...
    public override void FixedUpdate()
    {
        ...
        {
            rigidbody.linearVelocity += new Vector2(playerData.Physics.WallDirection, 0f);
        }
    ...
    public void Move1D()
    {
        ...
        rigidbody.linearVelocity = newVelocity * Vector2.up;
    }
    private float VelocityOnY()
    {
        float YVelocity;
        if (inputManager.Input_WallClimb != 0 && (rigidbody.linearVelocity.y == 0 || Mathf.Sign(inputManager.Input_WallClimb) == Mathf.Sign(rigidbody.linearVelocity.y))
            && (phase != Phase.TurnBack || curveTime > playerData.Walls.WallClimb.TurnBackTime))
        {
            if (phase != Phase.SpeedUp)
            {
                curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallClimb.SpeedUpCurve, Mathf.Abs(rigidbody.linearVelocity.y) / playerData.Walls.WallClimb.MaxSpeed, 1f, true);
                curveTime *= playerData.Walls.WallClimb.SpeedUpTime;
                phase = Phase.SpeedUp;
            }
        ...
        else if (inputManager.Input_WallClimb != 0 && ((rigidbody.linearVelocity.y != 0 && Mathf.Sign(inputManager.Input_WallClimb) != Mathf.Sign(rigidbody.linearVelocity.y)) || phase == Phase.TurnBack))
        {
            if (phase != Phase.TurnBack)
            {
                curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallClimb.TurnBackCurve, Mathf.Abs(rigidbody.linearVelocity.y) / playerData.Walls.WallClimb.MaxSpeed, 2f, false);
                curveTime *= playerData.Walls.WallClimb.TurnBackTime;
                phase = Phase.TurnBack;
                turnBackStartDirection = (int)Mathf.Sign(rigidbody.linearVelocity.y);
            }
        ...
        else
        {
            ...
            {
                curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallClimb.SlowDownCurve, Mathf.Abs(rigidbody.linearVelocity.y) / playerData.Walls.WallClimb.MaxSpeed, 1f, false);
            ...
            YVelocity *= playerData.Walls.WallClimb.MaxSpeed * Mathf.Sign(rigidbody.linearVelocity.y);
        }
