...
public class PlayerLandState : MainState, IMove2D
{
    ...
    {
        base.Enter();
        rigidbody.gravityScale = playerData.Land.Physics2DGravityScale;
    ...
    public override void FixedUpdate()
    {
        ...
        {
            rigidbody.linearVelocity = new(0f, -1f);
        }
    ...
    public void Move2D()
    {
        ...
        if (!playerData.Physics.IsOnNotWalkableSlope || playerData.Physics.FacingDirection != Mathf.Sign(playerData.Physics.ContactPosition.x - rigidbody.position.x))
        {
            _newVelocity.x = VelocityOnx();
        }
        rigidbody.linearVelocity = _newVelocity;
        localXVelovity = rigidbody.linearVelocity.x;
    }
    ...
    private float VelocityOnx()
    {
            ...
            && (phase != Phase.TurnBack || xCurveTime > playerData.Walk.TurnBackTime))
        {
            ...
            {
                xCurveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Land.XSpeedUpCurve, Mathf.Abs(rigidbody.linearVelocity.x) / playerData.Land.MaxXSpeed, 1, true);
        ...
        else if (localXVelovity != 0)
        {
            ...
            {
                xCurveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Land.XSlowDownCurve, Mathf.Abs(rigidbody.linearVelocity.x) / playerData.Land.MaxXSpeed, 1, false);
