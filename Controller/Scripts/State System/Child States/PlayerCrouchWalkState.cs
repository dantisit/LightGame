...
public class PlayerCrouchWalkState : MainState, IMove1D
{
    ...
    {
        base.Enter();
        rigidbody.gravityScale = playerData.Walk.Physics2DGravityScale;
        localTime = 0f;
        curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Crouch.SpeedUpCurve, rigidbody.linearVelocity.x / playerData.Crouch.MaxSpeed, 1, true);
    ...
    public void Move1D()
    {
        ...
        else if (!playerData.Physics.IsOnNotWalkableSlope)
        {
            ...
            rigidbody.linearVelocity = -1 * newVelocity * playerData.Physics.WalkSpeedDirection.normalized;
        }
        else
        {
            newVelocity = VelocityOnX();
            rigidbody.linearVelocity = new Vector2(newVelocity, rigidbody.linearVelocity.y);
        }
    ...
    private float VelocityOnX()
    {
        ...
        {
            rigidbody.gravityScale = playerData.Land.MinLandSpeed / Physics2D.gravity.y;
        }
        else
        {
            rigidbody.gravityScale = playerData.Crouch.Physics2DGravityScale;
        }
