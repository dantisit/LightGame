...
public class PlayerJumpState : MainState, IMove2D
{
    ...
    private float VelocityOnx()
    {
            ...
            && (phase != Phase.TurnBack || xCurveTime > playerData.Walk.TurnBackTime))
        {
            ...
            {
                xCurveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Jump.XSpeedUpCurve, Mathf.Abs(rigidbody.linearVelocity.x) / playerData.Jump.MaxXSpeed, 1, true);
        ...
        else if (localXVelovity != 0)
        {
            ...
            {
                xCurveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Jump.XSlowDownCurve, Mathf.Abs(rigidbody.linearVelocity.x) / playerData.Jump.MaxXSpeed, 1, false);
