...
public class PlayerWallJumpState : MainState, IMove2D
{
    ...
    public void Move2D()
    {
        ...
        if (localTime < playerData.Walls.WallJump.DampingTime)
        {
            ...
            rigidbody.linearVelocity = _newVelocity + xStartVelocity;
        }
    ...
    private float VelocityOnx()
    {
        ...
        if (inputManager.Input_Walk != 0)
        {
            ...
            {
                xCurveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallJump.XSpeedUpCurve, Mathf.Abs(rigidbody.linearVelocity.x) / playerData.Walls.WallJump.XMaxSpeed, 1, true);
        ...
        else
        {
            ...
            {
                xCurveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallJump.XSlowDownCurve, Mathf.Abs(rigidbody.linearVelocity.x) / playerData.Walls.WallJump.XMaxSpeed, 1, false);
