...
public class PlayerWallSlideState : MainState, IMove1D
{
    ...
    public override void Enter()
    {
        ...
        localTime = 0f;
        curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallSlide.SpeedUpCurve, rigidbody.linearVelocity.x / playerData.Walls.WallSlide.MaxSpeed, 1, true);
    ...
    {
        float YVelocity = 0;
        if (rigidbody.linearVelocity.y > 0 || (phase == Phase.TurnBack && curveTime <= playerData.Walls.WallSlide.TurnBackTime))
        {
            if (phase != Phase.TurnBack)
            {
                curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallSlide.TurnBackCurve, Mathf.Abs(rigidbody.linearVelocity.y) / playerData.Walls.WallSlide.MaxSpeed, 2f, false);
        ...
        else if (!inputManager.Input_WallGrab || playerData.Walls.CurrentStamina == 0)
        {
            ...
            {
                curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallSlide.SpeedUpCurve, Mathf.Abs(rigidbody.linearVelocity.y) / playerData.Walls.WallSlide.MaxSpeed, 1f, true);
                curveTime *= playerData.Walls.WallSlide.SpeedUpTime;
                phase = Phase.SpeedUp;
            }
        ...
        else if (inputManager.Input_WallGrab && playerData.Walls.CurrentStamina > 0)
        {
            ...
            {
                curveTime = EssentialPhysics.SetCurveTimeByValue(playerData.Walls.WallSlide.SlowDownCurve, Mathf.Abs(rigidbody.linearVelocity.y) / playerData.Walls.WallSlide.MaxSpeed, 1f, false);
