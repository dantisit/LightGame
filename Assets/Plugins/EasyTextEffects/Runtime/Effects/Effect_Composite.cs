...
namespace EasyTextEffects.Effects
{
    ...
    public class Effect_Composite : TextEffectInstance
    {
        ...
        public override TextEffectInstance Instantiate()
        {
            ...
            {
                if (!effect) continue;
                TextEffectInstance childInstance = effect.Instantiate();
                instance.effects.Add(childInstance);
            }

            return instance;
        }
        
        public override void SetTextEffect(EasyTextEffects.TextEffect textEffect)
        {
            base.SetTextEffect(textEffect);
            
            // Pass TextEffect reference to all child effects
            foreach (TextEffectInstance effect in effects)
            {
                if (!effect) continue;
                effect.SetTextEffect(textEffect);
            }
        }
